...
export const inputClassName =
  "flex h-10 w-full rounded-lg border border-line-strong bg-white px-3.5 text-sm text-ink transition-colors placeholder:text-ink-faint focus:border-brand-500 focus:outline-none focus:ring-3 focus:ring-brand-500/15 disabled:cursor-not-allowed disabled:bg-surface-3 disabled:opacity-70 aria-invalid:border-danger aria-invalid:focus:ring-danger/15";
...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, invalid, ...props }, ref) => (
  <textarea
    className={cn(inputClassName, "h-auto min-h-[104px] resize-y py-2.5 leading-relaxed", className)}
    ref={ref}
    aria-invalid={invalid || undefined}
    {...props}
  />
